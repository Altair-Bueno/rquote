...
async fn fetch_data(character: &str, page: u32) -> Result<Vec<AnimechanQuote>, Rc<dyn Error>> {
    let response = AnimechanQuote::get_quote_character(character, page).await;
    match response {
        Ok(quote) => Ok(quote),
        Err(error) => {
...
pub fn character(props: &CharacterProp) -> Html {
    ...
    let fetcher = {
        ...
        use_async(async move { fetch_data(character.as_str(), *page).await })
    };
